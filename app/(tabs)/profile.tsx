...
function NoEntryIcon({ size = 16, color = '#666' }: { size?: number; color?: string }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 16 16" fill="none">
      <Circle cx="8" cy="8" r="6" stroke={color} strokeWidth="1.4" />
      <Line x1="4" y1="4" x2="12" y2="12" stroke={color} strokeWidth="1.4" strokeLinecap="round" />
    </Svg>
  );
}
...
export default function ProfileScreen() {
 ...
 useEffect(() => {
  if (!user) return;
  const blocksQuery = query(collection(db, 'blocks'), where('blockedBy', '==', user.uid));
  ...
  const handleUnblock = async (blockId: string) => {
    ...
    try {
      ...
      setErrorModal({
        visible: true,
        title: 'SOMETHING WENT WRONG',
        message: "We couldn't unblock this user. Please check your connection and try again.",
      });
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.card, { marginTop: 16 }]}>
        ...
        <View style={styles.divider} />
        <TouchableOpacity style={styles.infoRow} onPress={() => setShowBlockedModal(true)}>
          <NoEntryIcon size={16} color="#666" />
          <Text style={styles.settingLabel}>Blocked Users</Text>
          <View style={[styles.togglePill, { backgroundColor: blockedUsers.length > 0 ? '#008080' : '#e0e0e0' }]}>
            <Text style={styles.togglePillText}>{blockedUsers.length}</Text>
          </View>
        </TouchableOpacity>
        <View style={styles.divider} />
      ...
      <Modal visible={showBlockedModal} animationType="slide" transparent>
        <View style={styles.sheetOverlay}>
          <TouchableOpacity style={styles.sheetBackdrop} onPress={() => setShowBlockedModal(false)} />
          <View style={styles.sheet}>
            <View style={styles.sheetHandle} />
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.sheetContent}>
              <Text style={[styles.sheetTitle, fontsLoaded && { fontFamily: 'Rajdhani_700Bold' }]}>BLOCKED USERS</Text>
              {blockedUsers.length === 0 ? (
                <View style={styles.emptyBlocked}>
                  <Text style={styles.emptyBlockedText}>You haven't blocked anyone.</Text>
                </View>
              ) : (
                blockedUsers.map((b: any) => (
                  <View key={b.id} style={styles.blockedRow}>
                    <Text style={styles.blockedName} numberOfLines={1}>
                      {b.blockedUserName || b.blockedName || 'Unknown User'}
                    </Text>
                    <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  emptyBlocked: { backgroundColor: '#fff', borderRadius: 14, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: '#e8e8e8', marginBottom: 12 },
  emptyBlockedText: { fontSize: 13, color: '#a0b8b8' },
  blockedRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 12, marginBottom: 8, borderWidth: 1, borderColor: '#e8e8e8' },
  blockedName: { fontSize: 14, color: '#333', fontWeight: '500', flex: 1, marginRight: 12 },
  unblockBtn: { borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: '#008080' },
  unblockBtnText: { fontSize: 11, fontWeight: '700', color: '#008080', letterSpacing: 0.5 },
});